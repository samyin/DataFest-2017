...
search_count_top <- complete_search %>% group_by(srch_destination_name) %>% 
  summarize(srch_cnt = n()) %>% filter(srch_cnt > 10000) %>% 
...
```{r}
london <- fread("/Users/samyin/Desktop/ASADataFest2017\ Data/london.txt")
class(london) = "data.frame"
london <- london %>% select(ncol(.)) %>% as.matrix()
```
